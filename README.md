...
- Monitoring cities or store types that begin to deviate from the current stable pattern

- Testing promotions targeted by customer segment rather than across all customers
...
● Data Cleaning: Handling missing values, formatting fields, creating date hierarchies (month, quarter, year)

● Business Analysis: KPI definition, trend analysis, segmentation, interpreting results, storytelling with data
...
- Run A/B tests on promotions targeted to specific customer categories instead of all customers

- Analyze product combinations (market basket analysis) to identify higher-margin cross-sell opportunities
...
- Run deeper segmentation (by ticket size, purchase frequency, or loyalty status) to move from descriptive analytics to predictive insights

- Collect real transaction data instead of synthetic data to validate whether the observed stability across seasons and promotions holds true in practice
